...
async function handler(m, { conn, args, usedPrefix, command }) {
    if (confirmation[m.sender]) return m.reply('يتم التحويل الان')
...
`.trim()
    ...
    if (!(who in global.db.data.users)) return m.reply(`✳️ ده مش موجود ف قاعدة البيانات بتاعتي`)
    if (user[type] * 1 < count) return m.reply(`✳️  *${type}*  ده مش هيكفي للنقل`)
    let confirm = `
هل انت متاكد انك تريد النقل *${count}* _*${type}*_ a  *@${(who || '').replace(/@s\.whatsapp\.net/g, '')}* ? 
...
handler.tags = ['econ']
handler.command = ['payxp','paydi', 'تحويل', 'darxp','dardi',]
